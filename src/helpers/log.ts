...
function sameMembers(
  ...
  return Math.max(members1.length, members2.length) * 0.8 <= sameMemberCount;
}
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (const realmName of environment.realms) {
        ...
        const data = await tauriApi.getRaidLastLogs(lastLogId || 0, realmName);
      ...
      for (let log of unfilteredLogs.sort((a, b) =>
        ...
        if (validator.validRaidLog(log)) {
          const logData = await tauriApi.getRaidLog(log.log_id, log.realm);
...
function processLogs(
  ...
  for (const log of logs) {
    ...
    const bossId = id.raidBossId(log.encounter_data.encounter_id, difficulty);
    ...
    if (isValidWeeklyLog(log)) {
      ...
      if (weeklyChallengeRaidBossName === bossName) {
        if (!weeklyChallenge[difficulty]) {
          weeklyChallenge[difficulty] = new documentManager.weeklyChallenge({
    ...
    for (let character of log.members) {
      ...
      for (const combatMetric of environment.combatMetrics) {
        if (environment.isSpecCombatMetric(character.spec, combatMetric)) {
          ...
          const currentPerformance = characterDocument[combatMetric] || 0;
...
function filterRaidLogBugs(logs: RaidLogWithRealm[]): RaidLogWithRealm[] {
  return logs.reduce((acc, log) => {
    ...
    for (const bug of environment.logBugs) {
      switch (bug.type) {
        case "ignoreLogOfId":
          if (bug.id === fixedLog.log_id && bug.realm === fixedLog.realm) {
            return acc;
          }
          break;
        ...
        case "ignoreLogOfCharacter":
          for (const member of fixedLog.members) {
            if (bug.name === member.name && bug.realm === fixedLog.realm) {
              return acc;
            }
          }
        ...
        case "overwriteSpecOfCharacter":
          if (bug.logId === fixedLog.log_id && bug.realm === fixedLog.realm) {
            fixedLog.members = fixedLog.members.map((member: any) => {
